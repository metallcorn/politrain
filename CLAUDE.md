...
# ЕДИНАЯ ТОЧКА ТРИАЖА (не собирать SQL-запросами!):
#   GET /api/v1/admin/triage — открытые reports (полный снапшот + source/pool_id/user_answer),
#     открытые admin_feedback, комменты session_ratings за N дней (?days=3), vitals
#   POST /api/v1/admin/triage/resolve {"reports":[ids],"feedback":[ids]} — закрытие разобранного
# ПРАВИЛО разбора отчётов (строго соблюдать, не нарушать даже если кажется очевидным):
# 1. Читать ПОЛНЫЙ снапшот (через /admin/triage — там же user_answer из DE)
...
# Цель резерва ДИНАМИЧЕСКАЯ (правило юзера: не генерировать бесконечно): RESERVE_DAYS(=3) ×
# среднесуточное прохождение ЭТОГО юзера на ЭТОМ уровне за 7 дней (floor 10, cap 90). Таймер
# ДВАЖДЫ в сутки (04:30 и 16:30) — тяжёлый день выедает запас, вечерний прогон добивает до
# вечерней сессии (#162). До 3 раундов за прогон на (юзер × уровень+next_level).
...
backend/
  ...
  services/
    ...
    mistral.py     — обёртка над Mistral API;
                     `current_key()` — ключ из `app_settings` (таблица, задаётся из админки) ПЕРВЫМ, env-фолбэк,
                       кеш ~10с; смена ключа из UI БЕЗ рестарта/.env (протухший env-ключ 30.08 = 11 дней простоя молча);
                       админка: вкладка «Система» → статус ключа (живой 401-чек) + поле «обновить ключ» (валидируется
                       живым вызовом ДО сохранения — нельзя залочиться) + карточки здоровья + расход;
                       смоук: HARD FAIL если за 24ч ВСЕ вызовы Mistral 401 и 0 успехов (ловит мёртвый ключ в 1-й день);
...
- `DailyExercise` — задания дня, source:
  ...
  - `practice` — режим Повторение = SRS-ОЧЕРЕДЬ (решение юзера 2026-07-12): отдаёт ТОЛЬКО due-задания (next_review<=today, is_correct=True, source new/bonus/review_ai/topic_d), старейшие первыми; stats.practice_due — счётчик на TrainingPage; пусто → «Всё повторено ✅». Не случайный микс, не curriculum
  - SRS-ИНТЕРВАЛЫ AI-заданий (правка #176, «очередь улетела в космос»): верный с 1-го раза → +4д (`_SRS_FIRST_DAYS`, не +1д — приток в очередь падает в разы); ГРАДАЦИЯ — зрелое задание (интервал≥21д `_SRS_GRADUATE_DAYS`, пережило долгий перерыв) отвечено чисто первым разом → `next_review=NULL` навсегда (выучено, не гоняем). Словарь: градация при interval≥45д (`_VOCAB_GRADUATE_DAYS`) + streak≥3. Всё в submit_answer (routers/training.py)
...
### Прокачка уровня вверх (пул к следующему уровню, 2026-07-29)
Диагностика показала застревание на A2 (словарь 333 A2 vs 2 B1, темы numbers-dates 47/5дн, простой синтаксис). Правки — ВСЕ относительны `user.level`/`_next_level()`, никаких хардкод-уровней (работает A1→A2 так же как A2→B1):
- **Словарь вверх**: `_pick_new_vocab(user, db, seen, n)` — приоритет уровня `_next_level`, но только когда текущий освоен (≥40 слов streak≥1), иначе сначала добить текущий. `_ensure_vocab_pool` проверяет резерв ИМЕННО следующего уровня (агрегат маскировал пустую B1-полку) и генерит на `target_level`
...
- Обработка ошибок запросов: `errorMessage(err, fallback)` из `api/index.js` — различает ответ сервера (показывает `detail`) и сетевой сбой/SW (показывает «Сервер не отвечает, обнови страницу / очисти данные сайта»). Использовать в catch вместо `err.response?.data?.detail || '...'` — иначе сетевой сбой выглядит как «неверный пароль». ВАЖНО: Pydantic-422 кладёт detail МАССИВОМ объектов — errorMessage склеивает msg'и; сырой массив в тосте рендерился как НИЧЕГО («без ошибки не пропускает с простым паролем» — юзер думал, сайт сломан)
...
| order_words из ДВУХ предложений («Ile trwa seans? Teraz idę do kina.», #253) | Mistral слил два несвязанных предложения в один мешок слов | `_fix_order_words_exercise`: внутренний `?`/`!`/`.` (не в самом конце) → reject |
| Загадка сливает ответ однокоренным словом («podziękować» → dziękuję, #168) | stem-check ловил только `c_norm[:-1]`, приставка po- ломала совпадение | +проверка `c_norm[:5] in q_norm` для ответов ≥6 букв (корень как подстрока) |
| Мусорный маркер в вопросе «(NCB)» (#160) | Mistral лепит всё-капс аббревиатуру к вопросу | `_validate_batch` срезает `\s*\([A-Z]{2,5}\)` до фиксеров (легит-маркеры строчные/цифры) |
| MC: вопрос не определяет лицо («Где вы» → ответ «Jesteśmy/мы», #254) | русский вопрос неоднозначен по лицу, ответ в другом лице | Промт GRAMMAR: местоимение вопроса и лицо ответа обязаны согласоваться |
| Числительное не согласовано с сущ. («ma ___ lata (25)» → нужно «lat», #165) | Mistral ставит дефолтное «lata» независимо от числа | Промт: правило 2/3/4→им.мн (lata), 5+→род.мн (lat); генеративная ошибка, отдельные — деактивировать |
| Ошибки на исправление никогда не разгребаются (#161/#166) | error-retry брал СЛУЧАЙНЫЕ из бэклога → старая ошибка могла не всплыть никогда | `_error_retry_entries`: `order_by(completed_at.asc())` — старейшие первыми, детерминированный слив бэклога |
| Клик-слова копятся в склонённых формах (#163) | learn-word пишет форму как в предложении; normalize_vocab — ручной скрипт | Разово прогнан `normalize_vocab.py` (53 к лемме, 8 дедуп). TODO при жалобах: гонять периодически |
| Очередь повторений «улетела в космос» (practice_due 300+, #176) | КАЖДЫЙ верный ответ планировался на +1д → приток ~30/день > оттока | `_SRS_FIRST_DAYS=4` (первый интервал +4д) + ГРАДАЦИЯ (зрелое+чисто → next_review=NULL). Разово: раскидал 300 due по +15д (~20/день), выучил-градуировал зрелые |
| Загадка/задание с ответом всплыло СНОВА после фикса валидатора (#255 tata/tato) | Валидатор чинит только НОВУЮ генерацию; старые сохранённые DailyExercise крутятся в review_ai/practice | При системном фиксе валидатора — перечистить запланированные DE: прогнать fixers по `next_review IS NOT NULL` заданиям, битым → `next_review=NULL` |
| «Na stole leżą…» повторяется в translate несмотря на skeleton-дедуп (#175) | `_question_skeleton` считался по вопросу, а у translate вопрос РУССКИЙ | `_skeleton_for_item(d)`: для translate скелет по польскому `correct_answer`; заменил `_question_skeleton` во всех 4 местах generation.py |
| Одна тема доминирует (numbers-dates 47 против 1 у других за 5д, #172) | Ротация бинарная (был/не был за 7д); низкий score держал тему первой → перевыдача | Частотный штраф в `_select_topics_for_generation`: `Counter` заданий темы за 3 дня, sort key `freq//5` первым → перегруженные темы вниз |
| Вокатив однообразен («drogi przyjacielu» каждый раз, #173) | Нет разнообразия адресатов в промте | `_TOPIC_FOCUS["vocative"]`: список адресатов (имена м/ж, титулы, семья, роли) + запрет дефолта |
| judge_sentence с цифрой врёт («od dwa (2) lat»→true, #256/#170) | Mistral не тянет согласование числительных в true/false; `(N)` — артефакт fill_blank | `_fix_judge_sentence_exercise`: `\(\d+\)` в вопросе → reject; 29 записей пула деактивировано |
| Кэш объяснения не учитывает ОТВЕТ юзера (#174) | ключ = вопрос+ответ+is_correct; другая ошибка → чужое объяснение | user_answer в `_explain_cache_key`; `ExplainRequest.regenerate` обходит кэш и перезаписывает; кнопка «Переобъяснить заново» в UI |
| Загадка с нетипичным признаком («tenis na plaży», #171) | verifier ловил только фактически ложное, не «нетипичное/сбивающее» | WORD_DEFINITION_VERIFY_PROMPT: отвергать неканоничные/атипичные признаки |
| Дословно верный ответ уходит в «Мистраль-лотерею» (#247/#249/#154: то отвергнут, то принят) | `_norm` резал пунктуацию только С КОНЦА — запятая ВНУТРИ эталона («Panie doktorze, proszę…») ломала точное сравнение | `_norm` убирает пунктуацию везде (`_INNER_PUNCT_RE`) + схлопывает пробелы |
| order_words: одно предложение дважды как две пул-записи (#247/#249) | question = перемешка; две разные перемешки одного предложения = разные question_norm | `_dedup_question_key`: для order_words ключ = НОРМ ПРЕДЛОЖЕНИЯ (первая альтернатива correct_answer); применён в _save_to_pool/_seen_questions/_validate_batch; разовый пересчёт норм пула (589 обновлено, 20 дублей снято) |
| Одни и те же 2-3 темы день за днём (vocative/numbers-dates, #149/#150) | После EMA-пересчёта не-done тем мало; fallback-порядок ротации брал ВЧЕРАШНИЕ (stale) раньше отдохнувших done | Порядок пулов: fresh → done_review → stale (недавние — последний резерв) |
| «Почти верно» на нормальных ответах (#151-153) | focus-правило в TRANSLATION_CHECK сделало Мистраля строже КО ВСЕМУ | В промт добавлен counter-example: синонимы ВНЕ фокуса допустимы (codziennie vs każdego dnia при фокусе się); строгость — только к дриллируемой конструкции |
| daily 14/20 при deficit=0 (#148) | Добор жил только в ветке генерации; пул закрыл ai_target сам, topic_d дал 0 → добирать некому | Финальный top-up из пула ПОСЛЕ сборки всех entries: countable<count → `_pool_draw(shortfall)` (ночной джоб держит пул полным — дёшево) |
| Сессии короче заявленных: daily 14/20, bonus 17/20 (#145/#146) | (1) daily резервировал 6 фикс-слотов под topic_d+новые слова, но topic_d может дать 0 (темы ушли в done), а vocab-карточки не входят в счётчик — резерв сгорал; (2) отбраковка сырой генерации 30-50%, overshoot 1.5× не покрывал | (1) динамический добор: `allowed = deficit + max(0, 4 - len(topic_d_entries)) + 2` — незанятые слоты добираются из уже сгенерированного излишка; (2) overshoot 2× (`min(deficit*2, deficit+16)`). Диагностика: `grep bonus_pool /tmp/uvicorn.log` — смотреть pool/deficit |
...
| Репортнутое задание ВОЗВРАЩАЕТСЯ снова и снова (#187 «я репортнул, почему оно блять снова?») | Репорт помечал ОДНУ строку DE верной + деактивировал пул, но КОПИИ того же задания (error-retry `is_error_retry`, review_ai, запланированные на повтор) — отдельные DE-строки, продолжали всплывать | В `/report` после деактивации пула — вычистить ВСЕ копии юзера по нормированному вопросу: `is_correct=True, is_completed=True, next_review=None` (скан только errors/review/pending, не вся история) |
| MC: факт-галлюцинация про язык («который месяц женского рода» → marzec, ВСЕ месяцы мужские) | Мистраль врёт в мета-вопросах О языке | GRAMMAR-промт: запрет мета-вопросов (какое написание/форма/род правильные) — просить ИСПОЛЬЗОВАТЬ форму в предложении, не называть верную (#250/#259/#260) |
| MC: прилагательное согласовано по роду, но не по ПАДЕЖУ («kupiła ___ sukienkę» → zielona вместо zieloną, #261/#262) | После kupić/mieć/lubić сущ. в Biernik, прил. тоже; Мистраль ставил Mianownik, иногда верной формы даже нет в options | GRAMMAR-промт: явное правило Biernik-согласования + верная форма ОБЯЗАНА быть среди options |
| Экзамен генерил СЫРЫМ Мистралем мимо валидаторов (2026-08-06) | `/exam/task/{reading,grammar}` звал READING_TEXT_PROMPT/GRAMMAR_EXAM_PROMPT напрямую → те же баги (неверные ответы, мета-вопросы, буквы-метки), что чистили в упражнениях | `build_validated_reading` (общая с daily reading) + `build_exam_grammar` (pool-first валидированные MC + `_validate_batch`) в services/generation.py; exam grammar — ТОЛЬКО multiple_choice (фронт рендерит опции-кнопки); reading-грейдер сверяет `correct_answer` (валидированное), не `correct` |
