...
JUDGE_EXERCISES_PROMPT = (
    ...
    "1. Выбери тип ошибки из списка ниже\n"
    "2. Составь предложение где эта ошибка выглядит правдоподобно (не очевидно)\n"
    "3. Убедись что без ошибки предложение было бы грамматически верным\n\n"
    ...
    "- Неверное согласование рода: \"dobry kobieta\" (→ dobra), \"nowy książka\" (→ nowa)\n"
    ...
    "- ВАЖНО: «ja byłem»/«ja byłam», «ja poszedłem»/«ja poszłam» — ВЕРНО (местоимение избыточно но не ошибка)\n"
    "- ВАЖНО: «ja był»/«ja poszedł» — НЕВЕРНО (это 3-е лицо, не 1-е). Не путай эти два случая.\n\n"
    "Ответь ТОЛЬКО валидным JSON массивом без markdown:\n"
    "[\n"
...
LEXICAL_EXERCISES_PROMPT = (
    ...
    "FLASHCARD — карточка слова или идиомы:\n"
    "- question: ОДНО польское слово или ОДНА устойчивая фраза/идиома целиком, без ___ и пропусков\n"
    "- ЗАПРЕЩЕНО в question: две грамматические формы одного слова через / (мужской/женский, ед./мн.)\n"
    "  Выбирай одну словарную форму — инфинитив для глаголов, именительный ед.ч. для существительных\n"
    "- correct_answer: перевод на {native_language}; смысловой эквивалент, НЕ дословный\n"
    "- Если слово имеет несколько равнозначных переводов — перечисли через ' / '\n"
    "- ТОЛЬКО реально существующие идиомы — если не уверен, замени на слово\n"
    '- translation: для идиом — дословный перевод (букв. "..."); для слов — null\n'
    "- word_hints: null\n\n"
