...
GRAMMAR_EXERCISES_PROMPT = (
    ...
    "- FORBIDDEN: orthography/format quizzes — 'Która data jest poprawnie zapisana?', 'Która forma jest "
    "poprawna dla daty?', 'który miesiąc jest rodzaju żeńskiego?' — meta-questions ABOUT the language "
    "(which spelling/form/gender is right) test trivia, not usage, and Mistral gets the facts wrong "
    "(claimed marzec is feminine — ALL Polish months are masculine). Ask the user to USE the form in a "
    "sentence, not to name which is correct (#250/#259/#260)\n"
    "- ADJECTIVE-NOUN AGREEMENT must match CASE, not just gender. After kupić/mieć/lubić the noun is in "
    "biernik (accusative): feminine noun 'sukienkę' → adjective ALSO in biernik 'zieloną' (NOT nominative "
    "'zielona'). The correct option and the answer must be the accusative form, and it MUST be among the "
    "options (#261/#262: answer was nominative 'czerwona', accusative 'czerwoną' wasn't even offered)\n"
...
WORD_DEFINITION_VERIFY_PROMPT = (
    ...
    "sweet, not sour) OR the description is ambiguous (fits marchew, pietruszka and dynia alike) OR a "
    "detail is misleading/atypical (e.g. 'tenis ... na plaży' — tennis is played on a court, not a beach; "
    "the sand detail throws the learner off, #171). Reject any clue that is not the CANONICAL, typical "
